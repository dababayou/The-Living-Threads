...
class Media {
    ...
    this.program = new Program(this.gl, {
      ...
      vertex: `
        ...
        void main() {
          vUv = uv;
          vec3 p = position;
          // Wavy effect removed for flat geometric look
          p.z = 0.0;
  ...
  update(scroll: any, direction: string) {
    ...
    if (currentBend === 0) {
      ...
      // instead of stopping at H, fixing the "shifted up" flat edges.
...
class App {
  ...
  createGeometry() {
    ...
    // Required for Raycast to work without crashing
    if (this.planeGeometry.computeBoundingBox) {
  ...
  onTouchMove(e: any) {
    ...
    // Determine auto-scroll direction based on manual swipe
    if (Math.abs(deltaX) > 2) {
      this.autoScrollDirection = Math.sign(deltaX);
    }
  }
  onTouchUp(e: any) {
    this.isDown = false;
    ...
    const dist = Math.abs(this.start - x) + Math.abs(this.startY - y);
    
    // If very little movement occurred, treat it as a click
    if (dist < 10 && this.onMediaClick) {
      ...
      if (hits.length > 0) {
        const clickedPlane = hits[0];
        const media = this.medias.find((m: any) => m.plane === clickedPlane);
        if (media) {
  ...
  onWheel(e: any) {
    ...
    // Adapt auto-scroll direction to mouse wheel direction
    this.autoScrollDirection = Math.sign(delta);
  ...
  onCheck() {
    // Disabled snapping to allow continuous smooth auto-scrolling
  }
  onResize() {
    this.screen = {
  ...
  update() {
    // Constant slow adaptive auto-scroll movement when not dragging
    if (!this.isDown) {
      this.scroll.target += this.autoScrollDirection * 0.05; 
    }
...
export default function CircularGallery({
  ...
  const [activeMedia, setActiveMedia] = useState<{ image: string, text: string } | null>(null);
    ...
    resolveFont(font, fontUrl).then(resolvedFont => {
      ...
      app = new App(containerRef.current, {
        ...
        onMediaClick: (media: any) => {
          setActiveMedia({ image: media.image, text: media.text });
        }
      });
    });
  ...
  return (
    <>
      ...
      {/* Modal for viewing the enlarged clicked image */}
      {activeMedia && (
        <div 
          className="fixed inset-0 z-[200] flex flex-col items-center justify-center p-6 bg-background/90 backdrop-blur-md animate-in fade-in zoom-in-95 duration-300"
          onClick={() => setActiveMedia(null)}
          style={{ cursor: 'zoom-out' }}
        >
          <img 
            src={activeMedia.image} 
            alt={activeMedia.text} 
            className="max-w-full max-h-[75vh] object-contain shadow-2xl rounded-sm mb-6" 
          />
          <p 
            className="text-text text-3xl italic tracking-wide"
